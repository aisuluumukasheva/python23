"Наследование - принцип ООП, в котором мы можем унаследовать, переопределить и использовать в дочернем классе все атрибуты и методы родительского класса"
...
class C(A):
    """
    Наследвали все методы и атрибуты у класса А и переопределили метод method
    """
    def method(self):
...
obj_a = A()
obj_a.method() # Метод в классе A  -> ничего не меняет в дочернем классе
...
print(B.mro()) #[<class '__main__.B'>, <class '__main__.A'>, <class 'object'>] [<class '__main__.B'>, <class '__main__.A'>, <class 'object'>]

# print(B.z) # AttributeError: type object 'B' has no attribute 'z'
...
"----------------------Виды наследования----------------------"
# одиночное наследование (когда мы наследуемся в дочернем классе от одного класса)
# множественное наследование (когда мы наследуемся в дочернем классе от нескольких классов )
# многоуровневое наследование (когда мы наследуемсяот класса, у которого есть родитель)
...
class C(A,B):
    "Наследовали все втрибуты у классов А и В"
    c = 'c'
