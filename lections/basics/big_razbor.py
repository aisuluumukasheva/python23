...
"=================Числа================="
# числа - неизменяемый тип данных, предназначенный для хранения числовых значений и преведения над ними арифметических действий
int # целые числа
...
"=====================СТРОКИ================"
# строка - неизменяемый тип данных, который представляет собой упорядоченную последовательность символов, заключенные в двойные или одинарные кавычки
...
# [start: end: step]
# [0 : len(iter) : 1]  от 0 до конца с шагом 1
string = 'some string'
...
#list1[1] == 'str'
list1[1] = 'hello'
# list1 = [5,'hello',(4,None), False, [1,2,3,4,5]]
...
"=================Set================="
# множества - изменяемый, неупорядоченный тип данных, который хранит в себе только уникальные значения
...
# ключи - должны быть неизменяемым типом данным
# если ключом будет tuple, то в нем тоже должны быть только неизменяемые типы данных)
# если ключ повторяется - значение переписывается на последнее
...
"=====================Conditions==============="
# условия - это конструкция, которая позволяет выполнять или не выполнять какой-то кусочек кода, который находится в теле условия ( то, что сдвинуто на tab)
...
"=================Тернарные выражения=============="
# условие, которое написано в одну строку и возвращает только лишь одно из двух результатов в зависимости от условия
"res1" if True else "res2"

"===================Циклы========================"
# цикл - это блок кода, который будет выполняться несколько раз, каждый круг называется итерацией
# (итерируемые объекты - те, по которым можно пройтись циклом for)
...
"--------------------For-------------------------"
# цикл, который проходится по итерируемым объектам (str, list, set, dict, tuple, range)
# В цикле for мы выполняем операции над каждым эементом итерируемого объекта
